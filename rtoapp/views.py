...
def add_learning_licence(request):
    ...
    if request.method == "POST":
        ...
        cityname = City.objects.get(id=cityname)
        Learning.objects.create(state=statename, city=cityname, father=father, register=reg, birth=birth, qualification=qualification, licencetype=licencetype, blood=blood,
                                    peraddress=peraddress, image2=image2, learningnumber=learningnumber, comaddress=comaddress)
        messages.success(request, "Add Successful Your Application No is"  + str(learningnumber))
        return redirect('view_learning_licence')
    mystate = State.objects.all()
    return render(request, 'add_learning_licence.html', locals())
...
def add_driving_licence(request):
    ...
    if request.method == "POST":
        ...
        statename = State.objects.get(id=name)
        cityname = City.objects.get(id=cityname)
        Driving.objects.create(state=statename, city=cityname, learningnumber=learningnumber, father=father, register=reg, birth=birth, qualification=qualification, licencetype=licencetype, blood=blood,
                                    peraddress=peraddress, image2=image2, drivingnumber=drivingnumber, comaddress=comaddress)
